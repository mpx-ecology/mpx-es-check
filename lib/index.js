...
const resultOutputPath = getLogOutputPath()
const acornOpts = { ecmaVersion: 12, silent: true, locations: true }

function parseCode (esVersion, esmodule, files, useAllRules) {
  ...
  files.forEach((pattern, index) => {
    ...
    globbedFiles.forEach((file, index) => {
      pb.render({ completed: index + 1, total: globbedFiles.length > files.length? globbedFiles.length: files.length  })
