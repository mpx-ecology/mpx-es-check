...
class EsCheckPlugin {
  ...
  apply (compiler) {
    ...
    const allBlockProblems = []
    const allNonBlockProblems = []
    ...
    compiler.hooks.afterEmit.tapPromise({
      ...
      const esCheckFilePath = path.resolve(compiler.outputPath, this.options.filename || 'es-check.log')
      if (this.options.callback && allNonBlockProblems.length > 0) {
        this.options.callback(allNonBlockProblems, this.options, compilation)
      }
      if (allBlockProblems.length) {
        // 处理对应的语法报错，收集输出到对应的文件夹
        const logger = compilation.getLogger('EsCheckPlugin')
        await writeFilePromise(esCheckFilePath, JSON.stringify(allBlockProblems, null, 2))
